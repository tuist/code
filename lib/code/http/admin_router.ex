defmodule Code.HTTP.AdminRouter do
  @moduledoc """
  ...
  get "/health" do
    send_json(conn, 200, %{status: "ok", node: Code.Config.node_id()})
  end
  ...
  get "/metrics" do
    ...
    |> send_resp(200, PromEx.get_metrics(Code.PromEx))
  end
  ...
  post "/compact/*repo" do
    ...
    case Code.Maintenance.run(repo_id, :compact) do
  ...
  post "/lookup/*repo" do
    ...
    case Code.Maintenance.run(repo_id, :lookup) do
      {:ok, summary} -> send_json(conn, 200, summary)
  ...
  post "/reap" do
    send_json(conn, 200, %{evicted: Code.Replica.Reaper.sweep()})
  end
