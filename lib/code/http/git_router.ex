defmodule Code.HTTP.GitRouter do
  @moduledoc """
  ...
  defp advertise(conn, repo_id, _service) do
    ...
    case conn.query_params["service"] do
      ...
      _ ->
        # Dumb HTTP would be served here. Code does not implement it: it
        # cannot express partial clone or protocol v2, and every Git built this
        # decade speaks smart HTTP.
        conn
        |> put_resp_content_type("text/plain")
        |> send_resp(400, "code: only the smart HTTP protocol is supported\n")
    end
  end
  ...
  defp run(conn, view, repo_id, "git-receive-pack" = service) do
    ...
    Code.Git.Hooks.install(view.path, repo_id)

    env =
      protocol_env(conn) ++
        [
          {"CODE_HOOK_URL", Code.Git.Hooks.callback_url()},
  ...
  defp fresh(conn, repo_id) do
    case Replica.ensure_fresh(repo_id) do
      ...
      {:error, :no_such_repository} ->
        {:error, send_resp(conn, 404, "code: repository not found\n")}

      {:error, reason} ->
        Logger.error("could not serve repository", repo_id: repo_id, reason: reason)
        {:error, send_resp(conn, 503, "code: repository temporarily unavailable\n")}
    end
  end
    ...
    case get_req_header(conn, "content-type") do
      [type | _] ->
        if String.starts_with?(type, expected) do
          ...
          {:error, send_resp(conn, 415, "code: expected #{expected}, got #{type}\n")}
        end
