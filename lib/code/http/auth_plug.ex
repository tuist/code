defmodule Code.HTTP.AuthPlug do
  @moduledoc """
    ...
    case Auth.authenticate(credential) do
      {:ok, principal} ->
        ...
        |> Code.Telemetry.put_span_attributes(%{
          ...
          "code.auth.source" => to_string(principal.source)
        })
  ...
  """
  @spec authorize(Plug.Conn.t(), String.t(), Code.Auth.Principal.permission()) ::
        ...
        case Auth.authorize(principal, repo_id, permission) do
          ...
          {:error, :forbidden} ->
            ...
            case Auth.authorize(principal, repo_id, :read) do
              :ok -> {:halt, deny(conn, 403, "code: not permitted to #{permission} #{repo_id}")}
              {:error, :forbidden} -> {:halt, deny(conn, 404, "code: repository not found")}
            end
        end
    end
  end
  ...
  @spec authorize_account(Plug.Conn.t(), String.t(), Code.Auth.Principal.permission()) ::
          {:ok, Plug.Conn.t()} | {:halt, Plug.Conn.t()}
    ...
    case conn.assigns[:principal] do
      ...
      principal ->
        case Auth.authorize_account(principal, account, permission) do
          ...
          {:error, :forbidden} ->
            {:halt, deny(conn, 403, "code: not permitted to administer account #{account}")}
        end
    end
  end
  ...
  def challenge(conn) do
    ...
    challenges =
      if mcp?(conn) do
        ...
        [{"www-authenticate", ~s(Basic realm="code")}, {"www-authenticate", bearer}]
      end

    conn
    |> prepend_resp_headers(challenges)
    |> deny(401, "code: authentication required")
  end
  ...
  def resource_metadata_url(conn) do
    Code.Config.public_url(conn) <> "/.well-known/oauth-protected-resource"
  end
end
