defmodule Code.HTTP.MCPRouter do
  @moduledoc """
  ...
  which the specification permits: Code never initiates messages, so there
  ...
  def call(%{method: "GET"} = conn, _opts) do
    ...
    |> send_resp(405, "code: this server does not initiate messages")
  end
  ...
  defp request_opts(conn, principal) do
    opts = [principal: principal, public_url: Code.Config.public_url(conn)]
