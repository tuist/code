defmodule Code.Auth.OIDC do
  ...
  authorization spec calls out. A token that does not name Code is rejected
    ...
    case Keyword.get(config, :issuer) do
      ...
      _ ->
        if Keyword.get(config, :kubernetes, false) do
          case Code.Auth.JWKS.issuer(config) do
            {:ok, issuer} -> issuer
            {:error, _reason} -> nil
          end
        end
  ...
  defp from_claim(claims, config) do
    claim = Keyword.get(config, :grants_claim, "code_grants")
  ...
  defp key_for(kid, config) do
    case Code.Auth.JWKS.fetch(kid, config) do
      {:ok, jwk} -> {:ok, jwk}
