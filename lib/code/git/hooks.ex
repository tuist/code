defmodule Code.Git.Hooks do
  ...
  the point at which Code needs to act:
  ...
  the refs after the log accepted them. `Code.WAL` closes the first window
  ...
  defp script(repo_id) do
    """
    #!/bin/sh
    # Managed by Code. Regenerated on every materialization; do not edit.
    #
    ...
    response=$(printf '%s' "$payload" | curl \\
      --silent --show-error \\
      --max-time "${CODE_HOOK_TIMEOUT:-600}" \\
      --request POST \\
      --header "Content-Type: text/plain" \\
      --header "X-Code-Token: ${CODE_HOOK_TOKEN:-}" \\
      --header "X-Code-Repository: #{repo_id}" \\
      --header "X-Code-Quarantine: ${GIT_QUARANTINE_PATH:-}" \\
      ...
      echo "code: could not reach the local node to commit this push" >&2
      exit 1
    }
