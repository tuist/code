defmodule Code.Replica do
  ...
  def ensure_fresh(repo_id, opts \\ []) do
    Code.Telemetry.span(
      ...
      fn ->
        call(
          repo_id,
          {:ensure_fresh, opts, Code.Telemetry.context()},
  ...
  def handle_call({:ensure_fresh, opts, context}, _from, state) do
    ...
    {reply, state} =
      Code.Telemetry.with_context(context, fn ->
        Code.Telemetry.span(
          "code.replica.refresh",
          %{
            "code.repository.id" => state.repo_id
          },
  ...
  defp ensure_state_fresh(state, budget, now) do
    if state.index && budget > 0 && state.verified_at && now - state.verified_at < budget do
      :telemetry.execute([:code, :replica, :serve], %{}, %{repo_id: state.repo_id, path: :budget})
      {{:ok, view(state)}, state}
    else
      case refresh(state) do
        {:ok, state} ->
