defmodule Code.Control do
  ...
  defp do_create_repository(repo_id, opts) do
    case WAL.create(repo_id, opts) do
      {:ok, index} ->
        ...
        :telemetry.execute([:code, :repository, :created], %{}, %{repo_id: repo_id})
        {:ok, summarize(index)}
  ...
  def cluster_status do
    ...
    %{
      node: node(),
      node_id: Code.Config.node_id(),
      members: members,
      size: length(members),
      resident_repositories: length(Replica.resident()),
      version: Code.Application.version(),
      git_version:
        case Git.version() do
          {:ok, version} -> version
          _ -> nil
        end,
      object_store: Code.ObjectStore.backend() |> elem(0) |> inspect(),
      uptime_ms: uptime_ms()
    }
  end
