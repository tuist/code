defmodule Code.Config do
  ...
  `$callers` automatically, and `Code.Replica` copies its starter's
  ...
  def object_store do
    case get(:object_store, nil) do
      nil -> {Code.ObjectStore.Filesystem, root: expand_path("priv/data/object-store")}
  ...
  def auth, do: get(:auth, {Code.Auth.Allow, []})
  ...
  so it is preferred over anything configured; `CODE_PUBLIC_URL` exists for
  ...
  defp normalize_role!(role) do
    raise ArgumentError, "unknown Code role: #{inspect(role)}"
  end
