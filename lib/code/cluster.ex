defmodule Code.Cluster do
  ...
  def announce(repo_id, epoch, seq) do
    ...
    Code.Maintenance.announce(repo_id, epoch, seq)

    :telemetry.execute([:code, :cluster, :announce], %{seq: seq}, %{repo_id: repo_id, epoch: epoch})
    :ok
  end
  ...
  def handle_info({:wal_advanced, repo_id, epoch, seq, from}, state) do
    ...
    if Code.Replica.alive?(repo_id) or responsible?(repo_id) do
      Code.Replica.hint(repo_id, epoch, seq)
    end
  ...
  def handle_info({:nodeup, node, _info}, state) do
    ...
    :telemetry.execute([:code, :cluster, :nodeup], %{count: length(members())}, %{node: node})
    {:noreply, state}
  end
  ...
  def handle_info({:nodedown, node, _info}, state) do
    ...
    :telemetry.execute([:code, :cluster, :nodedown], %{count: length(members())}, %{node: node})
    {:noreply, state}
  end
