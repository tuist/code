defmodule Code.Git do
  @moduledoc """
  Every interaction with the `git` binary.

  Code deliberately does not reimplement Git. Object storage, delta
  ...
  by running the same client everyone else runs. What Code owns is *where
  ...
  def executable do
    Application.get_env(:code, :git_executable) || System.find_executable("git") ||
  ...
  @doc """
  Create a bare repository configured the way Code needs it.
  ...
  def write_tree(repo_path, base_rev, changes) do
    index = Path.join(System.tmp_dir!(), "code-index-" <> random_suffix())
  ...
  def commit_tree(repo_path, tree, parents, message, author) do
    ...
    env = [
      {"GIT_AUTHOR_NAME", author[:name] || "Code"},
      {"GIT_AUTHOR_EMAIL", author[:email] || "code@localhost"},
      {"GIT_COMMITTER_NAME", author[:name] || "Code"},
      {"GIT_COMMITTER_EMAIL", author[:email] || "code@localhost"}
    ]
  ...
  def run(path, args, opts \\ []) do
    ...
    :telemetry.execute([:code, :git, :command], %{duration_us: duration, bytes: byte_size(output)}, %{
