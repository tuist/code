defmodule Code.MCP.Server do
  ...
  defp server_info do
    %{name: "code", title: "Code", version: Code.Application.version()}
  end
  ...
  defp read_resource("code://" <> rest, principal, opts) do
    case String.split(rest, "/") do
      [account, name, "refs"] ->
        with {:ok, result} <-
               Tools.call("list_refs", %{"repository" => "#{account}/#{name}"}, principal, opts) do
          {:ok, [%{uri: "code://#{rest}", mimeType: "application/json", text: JSON.encode!(result)}]}
        end
      ...
      [account, name, "blob", ref | path] ->
        ...
        with {:ok, result} <- Tools.call("read_file", args, principal, opts) do
          {:ok, [%{uri: "code://#{rest}", mimeType: "text/plain", text: result.content}]}
        end
  ...
  defp instructions(opts) do
    url = Keyword.get(opts, :public_url) || Code.Config.public_url()

    """
    Code is a Git host. You can read, search and commit to repositories through these
