...
configure_git_auth() {
  ...
  PATH="$bin:$PATH" \
    CODE_TEST_GIT_LOG="$log" \
    CODE_TEST_METADATA="$metadata_file" \
    CODE_TEST_REGISTRATION="$registration_file" \
    "${SHELLSPEC_PROJECT_ROOT}/scripts/configure-code-git" --url https://git.example.com "$@"
  status=$?
