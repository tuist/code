defmodule Code.Auth.GitAuthTest do
  use ExUnit.Case, async: true

  alias Code.Auth.GitAuth
  ...
  test "validates and builds public browser-login settings" do
    assert %{issuer: "https://identity.example.com", username: "oauth2", scopes: ["openid", "profile"]} =
             GitAuth.build("code-developers", @opts)
  end
  ...
  test "defaults an empty username" do
    assert %{username: "oauth2"} = GitAuth.build("code-developers", Keyword.put(@opts, :username, ""))
  end

  for {name, key, value, message} <- [
        {:insecure_issuer, :issuer, "http://identity.example.com", "CODE_OIDC_ISSUER"},
        {:credentialed_issuer, :issuer, "https://user:pass@identity.example.com", "CODE_OIDC_ISSUER"},
        {:insecure_endpoint, :token_endpoint, "http://identity.example.com/token",
         "CODE_GIT_AUTH_TOKEN_ENDPOINT"},
        {:insecure_registration_endpoint, :registration_endpoint, "http://identity.example.com/register",
         "CODE_GIT_AUTH_REGISTRATION_ENDPOINT"},
        {:bad_redirect, :redirect_uri, "https://identity.example.com/callback", "CODE_GIT_AUTH_REDIRECT_URI"},
        {:empty_scopes, :scopes, "", "CODE_GIT_AUTH_SCOPES"},
        {:control_character_scope, :scopes, "openid\tprofile", "CODE_GIT_AUTH_SCOPES"},
        {:multiline_client_id, :client_id, "code\ndevelopers", "CODE_GIT_AUTH_CLIENT_ID"},
        {:bad_username, :username, "oauth user", "CODE_GIT_AUTH_USERNAME"}
      ] do
    test "rejects #{name}" do
      opts = if unquote(key) == :client_id, do: @opts, else: Keyword.put(@opts, unquote(key), unquote(value))
      client_id = if unquote(key) == :client_id, do: unquote(value), else: "code-developers"
  ...
  test "rejects a non-OpenID Connect or Kubernetes backend" do
    assert_raise ArgumentError, ~r/CODE_GIT_AUTH_CLIENT_ID/, fn ->
      GitAuth.build("code-developers", Keyword.put(@opts, :backend, "webhook"))
    end

    assert_raise ArgumentError, ~r/CODE_GIT_AUTH_CLIENT_ID/, fn ->
      GitAuth.build("code-developers", Keyword.put(@opts, :kubernetes, true))
    end
  end
end
