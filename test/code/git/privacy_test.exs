defmodule Code.Git.PrivacyTest do
  ...
  test "does not advertise or accept updates to Code's private references", %{root: root} do
    ...
    assert :ok = Git.update_refs(target, [%V1.RefCommand{ref: "refs/code/issues", new_oid: private_head}])

    assert {advertised, 0} = git(["ls-remote", target], source)
    refute advertised =~ "refs/code/issues"

    assert {output, status} = git(["fetch", "file://#{target}", "refs/code/issues"], source)
    assert status != 0
    assert output =~ "couldn't find remote ref"

    assert {output, status} = git(["push", target, "HEAD:refs/code/blocked"], source)
    assert status != 0
