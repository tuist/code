defmodule Code.MCP.ServerTest do
  ...
  setup %{repo: repo, namespace: namespace} do
    ...
    {:ok, principal: principal, opts: [principal: principal, public_url: "http://code.test"]}
  end
  ...
  describe "server/discover" do
    test "reports supported versions, capabilities and identity in one request", %{opts: opts} do
      ...
      assert result._meta["io.modelcontextprotocol/serverInfo"].name == "code"
  ...
  describe "initialize" do
    test "is still answered, because legacy clients cannot fall forward", %{opts: opts} do
      ...
      assert result.serverInfo.name == "code"
    end
  ...
  describe "reading" do
    ...
    test "clone_url hands back a usable URL", %{opts: opts, repo: repo} do
      ...
      assert result.structuredContent.http == "http://code.test/#{repo}.git"
    end
  end
  ...
  describe "resources" do
    ...
    test "reads refs through a resource uri", %{opts: opts, repo: repo} do
      {:reply, %{result: result}} = request("resources/read", %{"uri" => "code://#{repo}/refs"}, opts)
